...
import com.hy.im.codec.pack.message.RecallMessageNotifyPack;
...
import com.hy.im.common.enums.MessageErrorCode;
...
//        BeanUtils.copyProperties(messageReadedContent,messageReadedPack);
    ...
    public void recallMessage(RecallMessageContent content) {
        ...
        if(content.getConversationType() == ConversationTypeEnum.P2P.getCode()){
            ...
            long seq = redisSeq.doGetSeq(content.getAppId() + ":" + SeqConstants.MESSAGE + ":" + ConversationIdGenerate.generateP2PId(content.getFromId(),content.getToId()));
            ...
            for (String memberId : groupMemberId) {
                String toKey = content.getAppId() + ":" + SeqConstants.MESSAGE + ":" + memberId;
                OfflineMessageContent offlineMessageContent = new OfflineMessageContent();
                offlineMessageContent.setDelFlag(DelFlagEnum.DELETE.getCode());
                BeanUtils.copyProperties(content,offlineMessageContent);
